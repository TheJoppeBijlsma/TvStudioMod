...
public class StudioLight extends PaintableBlock {
	...
	public StudioLight(Settings settings) {
		super(Block.Settings.create(Material.STONE).breakInstantly().sounds(BlockSoundGroup.STONE).luminance(StudioLight.createLightLevelFromLitBlockState(15)));
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		return (BlockState)this.getDefaultState().with(FACING, ctx.getHorizontalPlayerFacing()).with(LIT, ctx.getWorld().isReceivingRedstonePower(ctx.getBlockPos()));
	}
